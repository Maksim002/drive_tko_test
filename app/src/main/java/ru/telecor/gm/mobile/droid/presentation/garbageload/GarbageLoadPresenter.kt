...
class GarbageLoadPresenter @Inject constructor(
    ...
    private fun getSortingPhotos(
        ...
        val getGarbagePhoto = list.firstOrNull { garbage -> garbage.type == modelType.toString() && item.size != 0}
    ...
    fun setCleatPhoto(photo: ProcessingPhoto){
        val cleatPhoto = list.indexOfFirst{ garbage -> garbage.type == PhotoType.LOAD_TROUBLE.toString()}
        if (cleatPhoto != -1) {
    ...
    private fun updateUI() {
        try {
            ...
            getListOfDefaultContainers().forEach {
                if (it.containerStatus != null){
                    viewState.setActionCompleted(false)
                }
            }
...
//        }
    ...
    fun taskDoneButtonClicked(volume: Double, idTask: Int, status: String) {
        setPickupValue(volume, idTask,status)
    }
    ...
    private fun setPickupValue(volume: Double, idTask: Int, status: String) {
        var nameStatus = 0
...
//                    viewState.showMessage(rm.getString(R.string.error_photo_required_after))
        ...
        val standResult = StandResult(
            localTaskCache.id.toLong(), routeInteractor.processingArrivalTime?: Date().time, Date().time, Date().time, containerStatuses,
            photosCount ?: 0,
...
//        )
        ...
        val standResults: MutableList<StandResult> = mutableListOf()
        val standResult = StandResult(
            localTaskCache.id.toLong(), routeInteractor.processingArrivalTime?: Date().time, Date().time, Date().time,
    ...
    fun onTroubleReasonChosen(taskStatus: List<StatusTaskExtended>, reason: ContainerFailureReason) {
        taskStatus.forEach {
            val status = ContainerStatus(
                it.id.toLong(), reason,
                it.containerType.id,
...
//        saveTaskDataDraft()
    ...
    fun detailedPhoto(itemMore: ArrayList<StatusTaskExtended>){
        if (itemMore.size != 0){
            if (itemMore.size < 2){
                conId = itemMore[0].id.toString()
